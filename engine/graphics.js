...
export class CameraComponent extends Component {
  constructor(name, x, y, z , width = 800, height = 600, zoom = 1) {
    super("camera");
    this.id = name;
    this.x = x;
    this.y = y;
  ...
  resize(){
    this.width = window.innerWidth;
    this.height = window.innerHeight;
    const xscale = this.width/this.nativeWidth;
    const yscale = this.height/this.nativeHeight;
    this.zoom = Math.max(Math.min(xscale,yscale),this.minZoom);
...
export class EntityRenderComponent extends RenderComponent {
  ...
  draw(ctx, transform, dt){
    ...
    for(const entity of this.entities){
      let component = entity.getComponent("render");
      if(component){
      component.draw(ctx, entity.getComponent("transform"),dt);
      }
    }
    ctx.restore();
  }
...
export class RenderSystem extends System{
  ...
  update(entities, dt) {
    ...
    for (const entity of sortedEntities) {
      const components = [];
      if(false && entity.getComponent("ui") ){
        components.push (entity.getComponent("ui"));
      }
