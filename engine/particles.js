...
export class ParticleBurstComponent extends Component {
  constructor(quantity, particleTypeId, origin, polygon, particleGenerator) {
   // super("particleBurst");
    super("particleEmitter");
    this.quantity
    this.emissionRate = 0.4;//emissionRate;
    this.particleType = particleTypeId;
    this.origin = origin || { x: 0, y: 0 };
    this.lifetime = 1000;
    //this.particles = [];// = particle;
    this.particleGenerator = particleGenerator ||defaultParticleGenerator
  }

  emit(dt) {
    const particles = [];
    for (let i = 0; i < this.quantity; i++){
        particles.push(this.particleGenerator(this.origin, this.polygon, this.lifetime, this.particleType));
